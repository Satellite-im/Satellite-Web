...
async function scanComponents(dirs, srcDir) {
  ...
  const scannedPaths = [];
  for (const {path: path$1, pattern, ignore = [], prefix, extendComponent, pathPrefix, level} of dirs.sort(sortDirsByPathLength)) {
    ...
    for (const _file of await globby2__default['default'](pattern, {cwd: path$1, ignore})) {
      ...
      const fileNameParts = fileName.toLowerCase() === "index" ? [] : scule.splitByCase(fileName);
      ...
      const componentName = scule.pascalCase(componentNameParts) + scule.pascalCase(fileNameParts);
...
const componentsModule = function() {
  ...
  nuxt.hook("build:before", async (builder) => {
    ...
    console.info("Discovered Components:", path__default['default'].relative(process.cwd(), componentsListFile));
  });
};
